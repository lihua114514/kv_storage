...
func (wb *WriteBatch) Put(key []byte, value []byte) error {
	...
	// 暂存 LogRecord
	logRecord := &data.LogRecord{Key: key, Val: value}
...
func (wb *WriteBatch) Delete(key []byte) error {
	...
	// 暂存 LogRecord
	logRecord := &data.LogRecord{Key: key, Type: data.LOG_RECORD_TYPE_DLETED}
	wb.pendingWrites[string(key)] = logRecord
	return nil
}
...
func (wb *WriteBatch) Commit() error {
	...
	finishedRecord := &data.LogRecord{
		...
		Type: data.LOG_RECORD_TXN_FINISHED,
	}
	...
	for _, record := range wb.pendingWrites {
		...
		if record.Type == data.LOG_RECORD_TYPE_NOMAL {
			wb.db.indexer.Put(record.Key, pos)
		}
		if record.Type == data.LOG_RECORD_TYPE_DLETED {
